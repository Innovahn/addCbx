...
class agreement(osv.osv):
    ...
    _columns = {
        'name': fields.selection([('Clasea','Clase A'),
                                    ('claseb','Clase B'),
                                    ('clasec','Clase C'),
				    ('canje','CANJE')], string="Nombre",required=True, help='Name that helps to identify the agreement'),
	...
	'numeromeses':fields.integer(string="Anticipo de Cuotas Mensuales", help="anticipo de cuotas Mensuales."),
    ...
    def generate_next_year_orders(self, cr, uid, ids, context={}):
        ...
        endDate = datetime(startDate.year, startDate.month, startDate.day)+relativedelta(months=result[meses.id])
        for agreement in self.browse(cr, uid, ids, context=context):
